...
public sealed class ObjectPool : MonoBehaviour {
	...
	[SerializeField] StartupPoolMode startupPoolMode;
 	[NullAllowed] [SerializeField] StartupPool[] startupPools;
    ...
    public static void CreatePool<T>(T prefab, int initialPoolSize, Transform parent = null) where T : Component {
        CreatePool(prefab.gameObject, initialPoolSize, parent);
	}

    public static void CreatePool(GameObject prefab, int initialPoolSize, Transform parent = null) {
		...
		if (prefab != null && !instance.pooledObjects.ContainsKey(prefab)) {
			...
			if (initialPoolSize > 0) {		
				while (list.Count < initialPoolSize) {
                    var obj = (GameObject)Object.Instantiate(prefab, parent);
					obj.SetActive(false);
					list.Add(obj);
				}
			}
	...
	public static T Spawn<T>(T prefab, Vector3 position) where T : Component {
		return Spawn(prefab.gameObject, position, Quaternion.identity).GetComponent<T>();
	}
        
	public static T Spawn<T>(T prefab) where T : Component {
		return Spawn(prefab.gameObject, Vector3.zero, Quaternion.identity).GetComponent<T>();
	}
	...
	public static GameObject Spawn(GameObject prefab, Vector3 position, Quaternion rotation) {
		...
		if (instance.pooledObjects.TryGetValue(prefab, out list)) {
			...
			obj = (GameObject)Object.Instantiate(prefab, position, rotation, parent);
			instance.spawnedObjects.Add(obj, prefab);
			#if UNITY_EDITOR
			Debug.LogWarning("Creating new object. Pool was not big enough.");
			#endif
			return obj;
		} else {
			Transform parent = instance.transform;
			obj = (GameObject)Object.Instantiate(prefab, position, rotation, parent);
			#if UNITY_EDITOR
			Debug.LogWarning("Creating new object. No pool found.");
			#endif
			return obj;
		}
	}
...
public static class ObjectPoolExtensions
{
    ...
    public static void CreatePool<T>(this T prefab, int initialPoolSize, Transform parent) where T : Component
    {
        ObjectPool.CreatePool(prefab, initialPoolSize, parent);
    }
    ...
    public static void CreatePool(this GameObject prefab, int initialPoolSize, Transform parent)
    {
        ObjectPool.CreatePool(prefab, initialPoolSize, parent);
    }
